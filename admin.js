...
function archiveRows(){const type=document.querySelector("#archiveTypeFilter")?.value||"",state=document.querySelector("#archivePublicFilter")?.value||"";return allOrders.filter(o=>o.completedImage&&(!type||normalizeWorkType(o.workType)===type)&&(!state||(state==="public"&&o.publicWork)||(state==="private"&&!o.publicWork)||(state==="featured"&&o.featuredWork))).sort((a,b)=>Number(Boolean(b.featuredWork))-Number(Boolean(a.featuredWork))||String(b.completedDate||b.updatedAtIso||"").localeCompare(String(a.completedDate||a.updatedAtIso||"")))}
